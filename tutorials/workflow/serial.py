...
class serial: # name modified
    ...
    def atom_mapping( self, edge=None, edges=None, bVerbose=False ):
        print('-----------------------')
    ...
    def _commands_for_transitions( self, simpath, job ):
        for i in range(1,81):
            if self.JOBqueue=='SGE':
                ...
                cmd3 = '$GMXRUN -s tpr.tpr -dhdl dhdl$SGE_TASK_ID.xvg -c confout$SGE_TASK_ID.gro'.format(simpath) # -c flag added
                cmd4 = 'cp dhdl$SGE_TASK_ID.xvg {0}/.'.format(simpath)
                cmd5 = 'cp confout$SGE_TASK_ID.gro {0}/.'.format(simpath) # added also copy the confout.gro file
