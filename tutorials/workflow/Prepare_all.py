...
class Prepare_all: # name modified
    ...
    def __init__(self, **kwargs):
        ...
        self.replicas = 3        
        self.simTypes = ['em','eq_nvt', 'eq', 'transitions'] # modified
        ...
        self.JOBsimtime = 1-00# hours
        self.JOBsimnode = 1# --nodes
        self.JOBsimtask = 1# --ntasks-per-node
        self.JOBsimcpu = 1 # --cpus-per-task
        self.JOBbGPU = True
        self.JOBsimgpu = 1 # --gpus-per-node
        self.JOBerror = 'err.err'
        self.JOBoutput = 'out.out'
        self.JOBmodules = []
        self.JOBsource = []
        self.JOBexport = []
        self.JOBgmx = 'gmx_mpi mdrun'
        self.JOBpartition = 'standard'
        self.systemName = "DSSB"
        ...
        for l in glob.glob('{0}/*'.format(self.proteinPath)):
            ...
            if '.itp' in fname: # posre or top
                if 'posre' in fname:
                    ...
                    top_file = open(self.proteinPath+'/'+fname)
                    content = top_file.readlines()
                    top_file.close()
                    if fname.startswith('topol_'):
                        self.protein['mols'].append(fname[6:-4])
                    else:
                        self.protein['mols'].append(content[2].split()[0])
    ...
    def _print_folder_structure( self ): # modified
        print('\n---------------------\nDirectory structure:\n---------------------\n')
    ...
    def _prepare_single_tpr( self, simpath, toppath, state, simType, nvtpath=None, empath=None, frameNum=0 ):
            ...
            for state in self.states:
                for r in range(1,self.replicas+1):
                    ...
                    if bLig==True:
                        ...
                        self._prepare_single_tpr( simpath, ligTopPath, state, simType, empath, nvtpath )
                    ...
                    if bProt==True:
                        ...
                        nvtpath = self._get_specific_path(edge=edge,wp=wp,state=state,r=r,sim='eq_nvt')
                        empath = self._get_specific_path(edge=edge,wp=wp,state=state,r=r,sim='em')
                        self._prepare_single_tpr( simpath, protTopPath, state, simType, empath, nvtpath )
        print('DONE')
                ...
                for r in range(1,self.replicas+1):            
                    ...
                    if bLig==True:
                        ...
                        job = pmx.jobscript.Jobscript(fname=jobfile,
                                        simnode=self.JOBsimnode,simtask=self.JOBsimtask,simcpu=self.JOBsimcpu,
                                        err=self.JOBerror,out=self.JOBoutput,jobname=jobname,modules=self.JOBmodules,source=self.JOBsource,
                                        gmx=self.JOBgmx,bGPU=self.JOBbGPU,partition=self.JOBpartition, simtime=self.JOBsimtime)
    ...
    def _commands_for_transitions( self, simpath, job ):
        for i in range(1,81):
            ...
            cmd3 = '$GMXRUN -s tpr.tpr -dhdl dhdl$SLURM_ARRAY_TASK_ID.xvg -c confout$SLURM_ARRAY_TASK_ID.gro'.format(simpath) # -c flag added
            cmd4 = 'cp dhdl$SLURM_ARRAY_TASK_ID.xvg {0}/.'.format(simpath)
            cmd5 = 'cp confout$SLURM_ARRAY_TASK_ID.gro {0}/.'.format(simpath) # added also copy the confout.gro file
            job.cmds = [cmd1,cmd2,cmd3,cmd4, cmd5]
    ...
    def _submission_script( self, jobfolder, counter, simType='eq' ):
        ...
        if simType=='transitions':
            cmd = '\'sbatch --array=1-80 jobscript{0}\'.format(i)'
    ...
    def _run_analysis_full( self, full_ana_path, stateApath, stateBpath, bVerbose=False ):
        fA = ' '.join( glob.glob('{0}/run*/transitions/*xvg'.format(stateApath)) )
        fB = ' '.join( glob.glob('{0}/run*/transitions/*xvg'.format(stateBpath)) )
        oA = '{0}/integ0.dat'.format(full_ana_path)
        oB = '{0}/integ1.dat'.format(full_ana_path)
        wplot = '{0}/wplot.png'.format(full_ana_path)
        o = '{0}/results.txt'.format(full_ana_path)
    ...
    def run_analysis( self, edges=None, bLig=True, bProt=True, bParseOnly=False, bVerbose=False ):
        ...
        for edge in edges:
            ...
            if bLig==True:
                wp='water'
                ## for all dhdl files
                stateApath = self._get_specific_path(edge=edge,wp=wp, state='stateA')
                stateBpath = self._get_specific_path(edge=edge,wp=wp, state='stateB')
                full_ana_path = '{}/analysis_all'.format(self._get_specific_path(edge=edge,wp=wp))
                create_folder(full_ana_path)
                self._run_analysis_full( full_ana_path, stateApath, stateBpath, bVerbose=bVerbose )
            if bProt==True:
                wp='protein'
                ## for all dhdl files
                stateApath = self._get_specific_path(edge=edge,wp=wp, state='stateA')
            ...
            for r in range(1,self.replicas+1):
                ...
                if bProt==True:
                    ...
                    ## for all dhdl files
                    stateApath = self._get_specific_path(edge=edge,wp=wp)
                    stateBpath = self._get_specific_path(edge=edge,wp=wp)
                    full_ana_path = '{}/analysis_all'.format(self._get_specific_path(edge=edge,wp=wp))
                    create_folder(full_ana_path)
                    self._run_analysis_full( full_ana_path, stateApath, stateBpath, bVerbose=bVerbose )
        print('DONE')
    ...
    def _summarize_results( self, edges ):
        ...
        for edge in edges:
            ...
            ##All dhdl files
            rowName = '{0}_all'.format(edge)
            rowNameWater = '{0}_{1}_all'.format(edge,'water')
            rowNameProtein = '{0}_{1}_all'.format(edge,'protein')
            dg = self.resultsAll.loc[rowNameProtein,'val'] - self.resultsAll.loc[rowNameWater,'val']
            erra = np.sqrt( np.power(self.resultsAll.loc[rowNameProtein,'err_analyt'],2.0) \
                            + np.power(self.resultsAll.loc[rowNameWater,'err_analyt'],2.0) )
            errb = np.sqrt( np.power(self.resultsAll.loc[rowNameProtein,'err_boot'],2.0) \
                            + np.power(self.resultsAll.loc[rowNameWater,'err_boot'],2.0) )
            rowName = '{}_all'.format(edge)
            self.resultsSummary.loc[rowName,'val'] = dg
            self.resultsSummary.loc[rowName,'err_analyt'] = erra
            self.resultsSummary.loc[rowName,'err_boot'] = errb
    def analysis_summary( self, edges=None ):
        ...
        for edge in edges:
            for wp in ['water', 'protein']:
                analysispath = '{}/analysis_all'.format(self._get_specific_path(edge=edge,wp=wp))
                resultsfile = '{0}/results.txt'.format(analysispath)
                res = self._read_neq_results( resultsfile )
                self._fill_resultsAll( res, edge, wp, r='all' )
